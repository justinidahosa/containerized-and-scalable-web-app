...
resource "aws_apigatewayv2_integration" "alb_health" {
  api_id                 = aws_apigatewayv2_api.api.id
  integration_type       = "HTTP_PROXY"
  integration_method     = "GET"
  integration_uri        = "${trim(var.target_alb_url, "/")}/health"
  payload_format_version = "1.0"
}
...
resource "aws_apigatewayv2_route" "health_api" {
  ...
  target             = "integrations/${aws_apigatewayv2_integration.alb_health.id}"
